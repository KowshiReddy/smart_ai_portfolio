...
const Header: React.FC<{ personalInfo: PersonalInfo }> = ({ personalInfo }) => (
  <header className="mb-2 flex flex-col md:flex-row items-center p-6 border-b-2 border-gray-200 dark:border-gray-700">
    <div className="md:mr-8 mb-4 md:mb-0">
      <Image
        src={personalInfo.image || "/default-profile.jpg"}
...
const Experience: React.FC<{ experience: ExperienceItem[] }> = ({ experience }) => (
  <section className="w-full p-8 border-b-2 border-gray-200 dark:border-gray-700">
    <h2 className="text-3xl font-bold mb-6 pb-3 text-gray-900 dark:text-gray-100 border-b-2 border-gray-300 dark:border-gray-600">
      Experience
    </h2>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      {experience.map((item, index) => (
        <div key={index} className="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6">
          <h3 className="text-2xl font-semibold text-gray-800 dark:text-gray-200">{item.position}</h3>
          <p className="text-lg text-gray-700 dark:text-gray-300">{item.company}</p>
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">{item.startDate} - {item.endDate}</p>
          ...
          {item.companyUrl && (
            <a 
              ...
              className="block mt-3 text-blue-600 dark:text-blue-400 hover:underline text-sm"
            >
              {item.companyUrl}
            </a>
          )}
  ...
  <section className="mb-2 p-6 border-b-2 border-gray-200 dark:border-gray-700">
    ...
    {education.map((item, index) => (
      <div key={index} className="mb-6">
        ...
        <a href={item.institutionUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline">
          {item.institutionUrl}
        </a>
      </div>
    ))}
